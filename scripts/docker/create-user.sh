...
if ! getent group rxtx-share-group > /dev/null 2>&1; then
    addgroup -g "$PGID" rxtx-share-group
fi

# Check if a user with PUID exists; if not, create it
if ! id -u rxtx-share > /dev/null 2>&1; then
    if ! getent passwd "$PUID" > /dev/null 2>&1; then
        adduser -u "$PUID" -G rxtx-share-group rxtx-share > /dev/null 2>&1
    else
